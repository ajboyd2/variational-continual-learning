...
import sys
diffusion = float(sys.argv[-3])#, int(sys.argv[-1])]
beam_size = int(sys.argv[-2]) #1
coreset_size = int(sys.argv[-1])
jump_bias = 0.2 #0.0
...
path = f'model_storage/long_permuted{"_upper_fixed" if upper_fixed else ""}_{f"cs_{coreset_size}" if coreset_size > 0 else ""}/{path_suffix}/'    # Path where to store files
